-- Exclude zero-priced ("NOT FOR SALE" / rental) archive pieces from the feed.
--
-- Some stores (Grain de Sell, Numero 13) publish archive pieces on Shopify at
-- price 0.00 with "NOT FOR SALE" descriptions. The cron mirrors them faithfully
-- into products.price as the exact literal '€0.00' (shopifyFetch.js formats
-- `€${n.toFixed(2)}`, so no '€0' / '€0,00' variants exist). They must not
-- surface anywhere on the site.
--
-- This is a READ-TIME exclusion only — no data is mutated and `hidden` is not
-- written (that column belongs to enrich), so reverting restores every row
-- instantly. NULL price stays visible: null means unknown, not unsellable.
--
-- Both RPCs are redefined so the feed and its total count stay in agreement;
-- redefining only one would leave get returning rows the count omits,
-- understating totals and stranding the tail of the feed beyond reachable pages.
--
-- The predicate is added at all three visibility WHERE sites:
--   get:   the `store_order` CTE (unaliased) and the `ranked` CTE (p.price)
--   count: the single WHERE
--
-- Bodies below are the live production definitions:
--   get_interleaved_products    ← scripts/sql/2026-05-28-add-image-url-2.sql
--   count_interleaved_products  ← scripts/sql/2026-05-21-interleaved-rpcs.sql
--                                 (never redefined since; the 05-28 migration
--                                  deliberately leaves it untouched)
-- with the minimum delta applied. get is DROP+CREATE'd to match the 05-28
-- handling, so its explicit grants are re-applied below. count uses
-- CREATE OR REPLACE (signature and bigint return unchanged), preserving its ACL.
--
