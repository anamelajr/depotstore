...
- **Backfill `scripts/backfillEraYear.mjs` (new)**: modeled on `scripts/backfillSeasonCodes.mjs` (dotenv, service role, dry-run default / `--apply`, `ORDER BY id` paging 1000). All rows (incl. hidden/sold). Apply groups changed rows by computed year, `update({era_year: Y}).in("id", chunk)` with `chunkArray(ids, 100)`. Idempotent; no CAS needed (any concurrent writer computes the identical function).
...
3. Backfill: dry-run (expect ~66–69% coverage) → sample review → `--apply` → re-run reports 0 pending. SQL coverage + Hedi-membership preview query (mirrors the three rules; expect ~35–45).
