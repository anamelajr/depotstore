...
- **`nav-height` is `56px` in `globals.css`** — must match `h-[56px]` on the desktop nav. Mobile nav stays at `h-[50px]` (the CSS variable is consumed only in desktop sticky-offset calculations).
